...
只读工具通过检查后可准入；会产生副作用的工具还需要显式 `CONFIRM`。准入记录绑定 manifest 与源码 SHA-256 指纹，任一文件变化都会把状态变成 `stale`。
...
执行前根据插件名与完整输入生成稳定计划键。相同计划再次提交时复用持久回执，不重复写文件。工具源码变化后，旧准入失效，执行会被拒绝，直到重新检查和准入。
