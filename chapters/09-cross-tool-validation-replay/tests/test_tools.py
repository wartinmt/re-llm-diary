...
class ToolTests(RuntimeCase):
    ...
    def test_31_document_idempotent(self):
        a=self.doc.write("k","a.md","x"); b=self.doc.write("k","a.md","x"); self.assertEqual(a.receipt_id,b.receipt_id); self.assertEqual((self.root/"docs/a.md").read_text(),"x")
    def test_31b_document_key_rejects_different_effect(self):
        self.doc.write("k","a.md","x")
        with self.assertRaises(ToolError): self.doc.write("k","a.md","DIFF")
    ...
    def test_37_index_idempotent(self):
        a=self.idx.register("k","t","a","h"); b=self.idx.register("k","t","a","h"); self.assertEqual(a.receipt_id,b.receipt_id); self.assertEqual(self.idx.record("t")["path"],"a")
    def test_37b_index_key_rejects_different_effect(self):
        self.idx.register("k","t","a","h")
        with self.assertRaises(ToolError): self.idx.register("k","t","b","x")
    ...
    def test_40b_delete_key_rejects_different_original(self):
        self.doc.write("k","a.md","x"); self.doc.delete("c","a.md","r1")
        with self.assertRaises(ToolError): self.doc.delete("c","a.md","r2")
    def test_40c_remove_key_rejects_different_original(self):
        self.idx.register("k","t","a","h"); self.idx.remove("c","t","r1")
        with self.assertRaises(ToolError): self.idx.remove("c","t","r2")
